...
# 20171101 - TRCM - Moved definitions from main 'rest.py' script to separate module
# 20171123 - TRCM - rstrip results text to reomve spurious carriage returns,
#                 - lowercase the servernames, and re.sub newlines and commas from desc field.
import time
import getopt
import urllib2
import re
...
　
    ...
    for ci in cidetails:
        ...
        results['CIName'] = ciname_tag.text.rstrip().lower() if ciname_tag is not None else ""
        results['UUID'] = ciname_tag.attrib['UUID'].rstrip() if ciname_tag is not None else ""
        ciclass_tag       = ci.find("{{{0}}}CIClass".format(namespace))
        results['CIClass'] = ciclass_tag.text.rstrip() if ciclass_tag is not None else ""
        status_tag        = ci.find("{{{0}}}Status".format(namespace))
        results['Status'] = status_tag.text.rstrip() if status_tag is not None else ""
        active_tag        = ci.find("{{{0}}}Active".format(namespace))
        results['Active'] = active_tag.text.rstrip() if active_tag is not None else ""
        groupown_tag        = ci.find("{{{0}}}OwningGroup".format(namespace))
        results['OwningGroup'] = groupown_tag.text.rstrip() if groupown_tag is not None else ""
        groupsup_tag        = ci.find("{{{0}}}SupportingGroup".format(namespace))
        results['SupportingGroup'] = groupsup_tag.text.rstrip() if groupsup_tag is not None else ""
        purpose_tag       = ci.find("{{{0}}}Purpose".format(namespace))
        results['Purpose'] = purpose_tag.text.rstrip() if purpose_tag is not None else ""
    return results
　
...
　
    ...
    for ci in cidetails:
        ...
        ciname_tag        = ci.find("{{{0}}}CIName".format(namespace))
        results[ci]['CIName'] = ciname_tag.text.rstrip().lower() if ciname_tag is not None else ""
        results[ci]['UUID'] = ciname_tag.attrib['UUID'].rstrip() if ciname_tag is not None else ""
        ciclass_tag       = ci.find("{{{0}}}CIClass".format(namespace))
        results[ci]['CIClass'] = ciclass_tag.text.rstrip() if ciclass_tag is not None else ""
        status_tag        = ci.find("{{{0}}}Status".format(namespace))
        results[ci]['Status'] = status_tag.text.rstrip() if status_tag is not None else ""
        active_tag        = ci.find("{{{0}}}Active".format(namespace))
        results[ci]['Active'] = active_tag.text.rstrip() if active_tag is not None else ""
        cidescription_tag = ci.find("{{{0}}}CIDescription".format(namespace))
        results[ci]['CIDescription'] = cidescription_tag.text.rstrip() if cidescription_tag is not None else ""
        results[ci]['CIDescription'] = re.sub('[\n,]+',' ',results[ci]['CIDescription'])
        purpose_tag       = ci.find("{{{0}}}Purpose".format(namespace))
        results[ci]['Purpose'] = purpose_tag.text.rstrip() if purpose_tag is not None else ""
    return results
　
